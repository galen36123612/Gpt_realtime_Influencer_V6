...
test("routes the exact failed-preview district and party-list transcripts", () => {
  const district = requireRoute("你幫我看看台北市中山區有哪些市議員");
  assert.deepEqual(district.route?.args, {
    district: "中山區",
    detail: "quick",
  });
  ...
  assert.equal(districtResult.result.constituency, 4);
  assert.equal(districtResult.result.count, 7);

  const party = requireRoute("那台北市有哪些民進黨市議員");
  assert.deepEqual(party.route?.args, {
    party: "民主進步黨",
    detail: "quick",
  });
  const partyResult: any = executeCouncilorTool(
    party.route!.forcedTool!,
    party.route!.args
  );
  assert.equal(partyResult.result.count, 18);

  const final = createLocalFinalAnswerResponse({ route: party.route });
  assert.match(final.instructions, /每位議員的姓名都必須各出現一次/);
});

test("routes natural public-trip wording and the 北投居 STT variant", () => {
  const activity = requireRoute("你有跟其他市議員有公開行程或公開站台嗎？");
  assert.equal(activity.route?.intent.type, "public_activity_list");
  assert.deepEqual(activity.route?.args, {
    hasPublicEventWithShen: true,
    detail: "quick",
  });

  const beitou = requireRoute("那北投居有哪些市議員");
  assert.deepEqual(beitou.route?.args, {
    district: "北投區",
    detail: "quick",
  });
});
...
test("routes 林亮君 birthday, education and background to profile detail", () => {
  ...
  const executed: any = executeCouncilorTool(route!.forcedTool!, route!.args);
  assert.equal(executed.result.found, true);
  assert.equal(executed.result.data[0].name, "林亮君");
  assert.ok(executed.result.data[0].birthDate);
  assert.ok(executed.result.data[0].education.length > 0);
});
...
test("does not apply a stale policy topic after a Village turn", () => {
